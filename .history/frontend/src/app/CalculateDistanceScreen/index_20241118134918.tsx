...
type FeedbackScreenParam = {
  FeedbackScreen: { raceId: number | undefined };
...
const CalculateDistanceScreen = () => {
  ...
  const navigation = useNavigation<NavigationProp<FeedbackScreenParam>>();
  ...
  const startRace = async () => {
    try {
      if (response && origin && destination && selectedVehicle) {
        setLoadingStart(true);
        const responseApi = await apiStartRace(response,origin,destination,selectedVehicle,clientId);
        setRace(responseApi);
        Toast.show({
          type: "success",
          text1: "Corrida iniciada com sucesso !",
        });
      }
    } catch (err) {
      ...
      Toast.show({
        type: "error",
        text1: "Ocorreu um erro ao iniciar a corrida !",
      });
  ...
  const finishRace = async () => {
    try {
      if (race) {
          setLoadingStart(true);
          const responseApi = await apiFinishRace(race);
          Toast.show({
            type: "success",
            text1: "A corrida foi finalizada com sucesso !",
          });
        navigation.navigate("FeedbackScreen", { raceId: race?.corridaid });
      }
    } catch (err) {
      Toast.show({
        type: "error",
        text1: "Ocorreu um erro ao finalizar a corrida !",
      });
    } finally {
      setLoadingStart(false);
    }
